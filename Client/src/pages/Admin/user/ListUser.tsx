...
export default function ListUser() {
  const accessToken: { accessToken: string; user: User } = useMemo(() => {
  ...
  const { data: listUser } = useQuery({
    queryKey: ["userList"],
    queryFn: () => getUserList(accessToken.accessToken),
  });

  const getDataListUser: any = useMemo(() => {
    return listUser?.data.data ? listUser.data.data : [];
  }, [listUser?.data]);
    ...
    <div className="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
      <table className="w-full border-collapse bg-white text-left text-sm text-gray-500">
        ...
        <tbody className="divide-y divide-gray-100 border-t border-gray-100">
          {userList &&
            userList.map((user) => {
              if (accessToken.user && user._id === accessToken.user._id) {
                return;
              }
              return (
